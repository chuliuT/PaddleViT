...
import torch
import timm
from swin_transformer_det import build_swin_det
from config import *

from swin_det_pytorch import SwinTransformer as SwinTorch


config = get_config('./configs/swin_tiny_patch4_window7.yaml')
print(config)
    ...
    for stage_idx in range(num_stages):
        ...
        for block_idx in range(depths[stage_idx]):
            ...
            layer_mapping = [
                ...
                (f'{th_b_prefix}.attn.relative_position_index', f'{pp_b_prefix}.attn.relative_position_index'),
...
def convert(torch_model, paddle_model):
    ...
    for th_name, pd_name in mapping:
        if th_name in th_params.keys(): # nn.Parameters
            if th_name.endswith('relative_position_bias_table') or th_name.endswith('relative_position_index'):
...
def main():
    ...
    paddle_model = build_swin_det(config)
    paddle_model.eval()
    ...
    torch_model = SwinTorch(patch_size=config.MODEL.TRANS.PATCH_SIZE,
                            embed_dim=config.MODEL.TRANS.EMBED_DIM,
                            depths=config.MODEL.TRANS.STAGE_DEPTHS,
                            num_heads=config.MODEL.TRANS.NUM_HEADS,
                            window_size=config.MODEL.TRANS.WINDOW_SIZE)
    state_dict = torch.load('./mask_rcnn_swin_tiny_patch4_window7.pth')
    #print(state_dict.keys())
    amp = state_dict['amp'] # loss scalar
    meta = state_dict['meta'] # class labels
    st = state_dict['state_dict']
    backbone_state_dict = OrderedDict()
    for key, val in st.items():
        if 'backbone' in key:
            backbone_state_dict[key[9::]] = val
    #print(amp)
    #print(meta)
    print('...................')
    print(backbone_state_dict.keys())
    ...
    out_torch_all = torch_model(x_torch)
    print('|||||||||||||||||||||||||||||||||||||||||||||||||||')
    print('|||||||||||||||||||||||||||||||||||||||||||||||||||')
    print('|||||||||||||||||||||||||||||||||||||||||||||||||||')
    out_paddle_all = paddle_model(x_paddle)

    assert len(out_torch_all) == len(out_paddle_all)
    
    for out_torch, out_paddle in zip(out_torch_all, out_paddle_all):
        ...
        model_path = os.path.join('./swin_tiny_patch4_window7.pdparams')
